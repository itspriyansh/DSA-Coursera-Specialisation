...
Matrix read_data(Graph& ch) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            int l = ch.query(vertices[i], vertices[j]);
            graph[i][j] = l == -1 ? INF : l;
        }
    }
    return graph;
...
int optimal_path(const Matrix& graph, int pos, int visit, Matrix &state) {
    // Implement this function yourself
    if(visit == ((1 << graph.size())-1))
        return graph[pos][0];

    if(state[pos][visit] != INF)
        return state[pos][visit];

    for(int i=0;i<graph.size();i++){
        if(i==pos || (visit & (1 << i)))
            continue;

        int dist = graph[pos][i] + optimal_path(graph, i, visit | (1 << i), state);
        if(dist < state[pos][visit])
            state[pos][visit] = dist;
    }
    return state[pos][visit];
}
...
int main() {
    ...
    for (int i = 0; i < t; ++i) {
        ...
        Matrix state(graph.size());
        for(auto& neighbours: state)
            neighbours = vector<int>((1 << graph.size())-1, INF);
        int result = optimal_path(graph, 0, 1, state);
        printf("%d\n", (result != INF && result>=0) ? result : -1);
    }
}
